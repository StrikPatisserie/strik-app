...
export default function LetterShopClient({ checkoutEnabled, pickupDates }: { checkoutEnabled: boolean; pickupDates: string[] }) {
  ...
  const canReview = cart.length > 0 && customerName.trim().length >= 2 && customerEmail.includes("@") && phone.trim().length >= 6 && Boolean(shop) && pickupDates.includes(pickupDate) && photoBytes <= 5_000_000 && cart.every((line) => !line.withLogo || Boolean(line.logoFile));

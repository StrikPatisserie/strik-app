...
export default function LetterShopClient() {
  ...
  return <main className="min-h-dvh bg-[#dbe8d7] text-[#3e312c]">
    ...
    <header className="relative overflow-hidden border-b border-white/30 px-4 pb-12 pt-5 sm:px-8 lg:px-12 lg:pb-20">
      <div aria-hidden="true" className="absolute -right-20 top-20 h-60 w-60 rotate-12 rounded-[4rem] bg-[#547762]/20 sm:h-80 sm:w-80" />
      <nav className="relative mx-auto flex max-w-7xl items-center justify-between gap-4"><Image src="/strik-logo.png" alt="Strik Patisserie" width={112} height={72} className="h-14 w-auto object-contain sm:h-16" priority /><button type="button" onClick={() => setCheckoutOpen(true)} className="rounded-full bg-white px-4 py-3 text-sm font-black text-[#3e312c] shadow-lg">Winkelmand <span className="ml-1 rounded-full bg-[#547762] px-2 py-1 text-xs text-white">{totalQuantity}</span></button></nav>
      <div className="relative mx-auto mt-12 max-w-7xl sm:mt-16"><p className="text-xs font-black uppercase tracking-[.25em] text-[#547762] sm:text-sm">Sinds 1937 · ambacht uit Nijmegen</p><h1 className="mt-3 text-[clamp(3.5rem,12vw,9rem)] font-black leading-[.82] tracking-[-.08em] text-[#3e312c]">LETTER<br className="sm:hidden"/>SHOP</h1><p className="mt-1 font-[Butterscotch] text-[clamp(2.5rem,8vw,6rem)] leading-none text-[#a6684b]">Met een Strik</p><p className="mt-7 max-w-2xl text-base font-bold leading-relaxed text-[#61504a] sm:text-xl">Kies jouw letter, chocolade en formaat. Wij maken hem met liefde; jij haalt hem op in één van onze vier winkels.</p><button type="button" onClick={() => document.getElementById("letter-assortiment")?.scrollIntoView({ behavior: "smooth" })} className="mt-7 rounded-full bg-[#547762] px-6 py-4 text-sm font-black text-white shadow-xl">Ontdek de letters ↓</button></div>
    </header>

    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-8 lg:px-12"><div className="grid gap-3 sm:grid-cols-3">{[{ number: "01", title: "Stel samen", text: "Letter, smaak en formaat kies je zelf." }, { number: "02", title: "Kies jouw winkel", text: "Ophalen in Ziekerstraat, Heyendaal, Daalseweg of Lent." }, { number: "03", title: "Betaal bij afhalen", text: "Geen online betaling nodig." }].map((item) => <div key={item.number} className="rounded-2xl border border-white/60 bg-[#f7f4e9] p-5"><span className="text-xs font-black tracking-[.2em] text-[#547762]">{item.number}</span><h2 className="mt-2 text-xl font-black">{item.title}</h2><p className="mt-1 text-sm font-semibold text-[#73584e]">{item.text}</p></div>)}</div></section>

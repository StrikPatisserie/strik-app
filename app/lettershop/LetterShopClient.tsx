...
export default function LetterShopClient({ checkoutEnabled, earliestPickupDate }: { checkoutEnabled: boolean; earliestPickupDate: string }) {
  ...
  const canReview = cart.length > 0 && customerName.trim().length >= 2 && customerEmail.includes("@") && phone.trim().length >= 6 && Boolean(shop) && Boolean(pickupDate) && (!checkoutEnabled || Boolean(earliestPickupDate) && pickupDate >= earliestPickupDate) && photoBytes <= 5_000_000 && cart.every((line) => !line.withLogo || Boolean(line.logoFile));

...
const PHOTOS = [
  ...
  { id: "vorm", label: "Vormletter S · melk, puur en wit", src: "/sinterklaas/vormletters S 2026.png", style: "vorm", chocolate: "melk" },
] as const;
...
export default function B2BChocolateLetters({
  ...
  return (
    <article id={`product-${productId}`} className="flex h-full flex-col overflow-hidden rounded-[1.35rem] bg-[#fff3cf] shadow-[0_12px_34px_rgba(107,35,12,.14)]">
      <div className="relative aspect-[3/4] shrink-0 overflow-hidden">
        ...
        {eyebrow && <span className="absolute left-2.5 top-2.5 rounded-full bg-[#d62d1d] px-2 py-0.5 text-[.56rem] font-black uppercase tracking-wider text-white">{eyebrow}</span>}
        <div className="absolute right-2.5 top-2.5 flex gap-1">
          <button type="button" onClick={onOpenShelfLife} aria-label={`Bekijk houdbaarheid van ${productName}`} title="Houdbaarheid" className="flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-[#557965] shadow-md backdrop-blur transition hover:bg-white"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="8.5"/><path d="M12 7.5V12l3 2"/></svg></button>
          <button type="button" onClick={onOpenAllergens} aria-label={`Bekijk allergenen van ${productName}`} title="Allergenen" className="flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-[#6e7858] shadow-md backdrop-blur transition hover:bg-white"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v18M12 7c-3 0-4-2-4-3 2.5 0 4 1 4 3ZM12 11c-3 0-5-2-5-4 3 0 5 1.5 5 4ZM12 15c-3 0-5-2-5-4 3 0 5 1.5 5 4ZM12 7c3 0 4-2 4-3-2.5 0-4 1-4 3ZM12 11c3 0 5-2 5-4-3 0-5 1.5-5 4ZM12 15c3 0 5-2 5-4-3 0-5 1.5-5 4Z"/></svg></button>
        </div>
      </div>
      <div className="flex flex-1 flex-col p-[.9rem]">
        <h3 className="text-lg font-black leading-tight text-[#60190f]">{productName}</h3>
        <p className="mt-1 min-h-9 text-[.68rem] font-semibold leading-relaxed text-[#7e493c]">{style === "vorm" ? "Grote vormletter S in melk, puur of wit." : "Spuitletters A–Z in melk, puur of wit. Kies klein of groot."}</p>
        <div className={`mt-1.5 grid gap-1 ${style === "spuit" ? "grid-cols-3" : "grid-cols-1"}`}>
          <label className="text-[.62rem] font-black">Chocolade<select value={chocolate} onChange={(event) => chooseChocolate(event.target.value as B2BLetterLine["chocolate"])} className="mt-0.5 h-8 w-full rounded-lg border border-[#e2c99c] bg-white px-2 text-[.68rem]"><option value="melk">Melk</option><option value="puur">Puur</option><option value="wit">Wit</option></select></label>
          {style === "spuit" && <label className="text-[.62rem] font-black">Letter<select value={letter} onChange={(event) => setLetter(event.target.value)} className="mt-0.5 h-8 w-full rounded-lg border border-[#e2c99c] bg-white px-2 text-[.68rem]">{LETTERS.map((item) => <option key={item}>{item}</option>)}</select></label>}
          {style === "spuit" && <label className="text-[.62rem] font-black">Formaat<select value={size} onChange={(event) => setSize(event.target.value as B2BLetterLine["size"])} className="mt-0.5 h-8 w-full rounded-lg border border-[#e2c99c] bg-white px-2 text-[.68rem]"><option value="klein">Klein</option><option value="groot">Groot</option></select></label>}
        </div>

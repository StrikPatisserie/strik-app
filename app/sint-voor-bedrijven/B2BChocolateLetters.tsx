...
function tierPrice(priceIncl: number, discountPercent: number, includeVat: boolean) {
  const discountedIncl = Math.round(priceIncl * (1 - discountPercent / 100) * 100) / 100;
  return includeVat ? discountedIncl : Math.round((discountedIncl / 1.09) * 100) / 100;
}
      ...
      <div className="grid md:grid-cols-[minmax(0,.8fr)_minmax(0,1fr)]">
        <div className="bg-[#f7f0e5]">
          <div className="relative mx-auto aspect-[2/3] max-h-[31rem] w-full">
            <Image src={selectedPhoto.src} alt={`Voorbeeld ${selectedPhoto.label.toLowerCase()}`} fill sizes="(max-width: 768px) 100vw, 40vw" className="object-contain" />
          ...
          <div className="grid grid-cols-4 gap-2 p-3">
            {PHOTOS.map((photo) => <button key={photo.id} type="button" onClick={() => choosePhoto(photo)} aria-label={photo.label} aria-pressed={photo.id === selectedPhoto.id} className={`overflow-hidden rounded-xl border-2 bg-white text-left ${photo.id === selectedPhoto.id ? "border-[#d62d1d]" : "border-transparent"}`}><Image src={photo.src} alt="" width={100} height={150} className="h-16 w-full object-cover" /><span className="block truncate px-1 pb-1 text-center text-[.6rem] font-bold">{photo.label}</span></button>)}
          </div>
        </div>
        <div className="p-5 sm:p-6">
          <h3 className="text-2xl font-black text-[#60190f]">Chocoladeletters</h3>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-[#7e493c]">Spuitletters A–Z in melk, puur of wit, klein of groot. Vormletter S in melk, puur of wit, alleen groot. Winkelprijs klein {money(8.95)}, groot {money(13.95)} incl. btw.</p>
          ...
          <details className="mt-3 rounded-xl border border-[#e2c99c] bg-white/70 p-3 text-sm"><summary className="cursor-pointer font-black">Speciaal verzoek (optioneel)</summary><div className="mt-2 flex flex-wrap gap-3">{REQUESTS.map((request) => <label key={request.id} className="flex items-center gap-1 text-xs font-bold"><input type="checkbox" checked={specialRequests.includes(request.id)} disabled={(request.id === "vegan" || request.id === "lactosevrij") && chocolate !== "puur"} onChange={(event) => setSpecialRequests((current) => event.target.checked ? [...current, request.id] : current.filter((item) => item !== request.id))} />{request.label}</label>)}</div><p className="mt-2 text-xs text-[#7e493c]">Vegan en lactosevrij alleen bij puur. Kan sporen van allergenen bevatten; bij ernstige allergieën raden we bestellen af.</p></details>
          <div className="mt-3 flex items-end gap-2"><label className="text-xs font-black">Aantal<input type="number" min="1" max="10000" value={quantity} onChange={(event) => setQuantity(Math.max(1, Math.min(10000, Number(event.target.value) || 1)))} className="mt-1 h-11 w-20 rounded-lg border border-[#e2c99c] bg-white text-center text-sm" /></label><button type="button" onClick={addLine} className="h-11 flex-1 rounded-lg bg-[#d62d1d] px-3 text-sm font-black text-white">+ Voeg variant toe</button></div>
          {lines.length > 0 && <div className="mt-4 rounded-xl bg-white p-3"><p className="mb-2 text-sm font-black">In jouw aanvraag · {totalQuantity} letters</p><div className="space-y-1">{lines.map((line) => <div key={line.id} className="flex items-center gap-2 border-t border-[#eee0c4] py-1.5 text-xs"><span className="min-w-0 flex-1 font-bold">{describeB2BLetter(line)}</span><input aria-label={`Aantal ${describeB2BLetter(line)}`} type="number" min="1" max="10000" value={line.quantity} onChange={(event) => onChange(lines.map((item) => item.id === line.id ? { ...item, quantity: Math.max(1, Math.min(10000, Number(event.target.value) || 1)) } : item))} className="w-16 rounded-md border border-[#e2c99c] p-1 text-center" /><button type="button" aria-label={`${describeB2BLetter(line)} verwijderen`} onClick={() => onChange(lines.filter((item) => item.id !== line.id))} className="px-1 text-lg font-black text-[#a32b1c]">×</button></div>)}</div></div>}
          <div className="mt-4 grid gap-2 text-xs font-bold text-[#60190f]"><label className="flex items-center gap-2"><input type="checkbox" checked={withLogo} onChange={(event) => onLogoChange(event.target.checked)} />Eigen logo op alle letters · +{money(logoUnitPrice)} p.s.</label><label className="flex items-center gap-2"><input type="checkbox" checked={giftWrap} onChange={(event) => onGiftWrapChange(event.target.checked)} />Alles in cadeaupapier · +{money(includeVat ? 1 : Math.round((1 / 1.09) * 100) / 100)} p.s.</label></div>
          <div className="mt-4 grid grid-cols-[auto_1fr_1fr] gap-x-2 gap-y-1 text-xs"><strong>Staffel</strong><strong>Klein · spuit</strong><strong>Groot · spuit/vorm</strong>{tiers.map((tier) => <div key={tier.label} className={`col-span-3 grid grid-cols-[auto_1fr_1fr] gap-2 rounded-lg px-2 py-1.5 ${totalQuantity > 0 && activeTierLabel === tier.label ? "bg-[#d62d1d] text-white" : "bg-white/75"}`}><span>{tier.label} st.</span><span>{money(tierPrice(8.95, tier.discountPercent || 0, includeVat))}</span><span>{money(tierPrice(13.95, tier.discountPercent || 0, includeVat))}</span></div>)}</div>
          <p className="mt-3 text-right text-sm font-bold">{totalQuantity ? `${totalQuantity} letters · ${money(total)}` : "Voeg een variant toe om het totaal te zien"}</p>
        </div>
      </div>
    </article>
  );

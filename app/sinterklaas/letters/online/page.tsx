  ...
  return <StrikShell wide>
    ...
    <div className="space-y-3">{orders.map((order) => {
      ...
      return <article key={order.id} className="rounded-2xl border border-[#e9ddd1] bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-black">{order.order_number} · {order.customer_name}</h2><p className="text-sm">{order.requested_date} · {order.pickup_location} · {order.fulfillment_status}</p><p className="text-xs text-[#776a5f]">{order.customer_email} · {order.phone}</p></div><strong>{money(total)}</strong></div>

...
export default async function OnlineLetterOrdersPage() {
  ...
  const { data, error } = await supabase.from("letter_orders")
    .select("id,order_number,customer_name,customer_email,phone,notes,requested_date,pickup_location,gift_wrap,fulfillment_status,created_at,letter_order_items(quantity,logo,logo_storage_path,unit_price_cents,logo_price_cents,letter_products(letter,flavour,size,style))")
  ...
  return <StrikShell wide>
    <StrikPageHeader title="Webshopbestellingen chocoladeletters" icon={strikIcons.sinterklaasLetter} />
    ...
    <div className="space-y-3">{orders.map((order) => {
      ...
      return <article key={order.id} className="rounded-2xl border border-[#e9ddd1] bg-white p-4 shadow-sm">
        ...
        {order.gift_wrap && <p className="mt-2 text-sm font-bold text-[#547762]">🎁 Alle letters in cadeaupapier · € 1 per letter</p>}
